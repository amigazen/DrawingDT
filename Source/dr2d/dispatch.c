...
/*****************************************************************************/
/* dispatch.c -- BOOPSI dispatcher for dr2d.datatype                         */
/*                                                                           */
/* dr2d.datatype is intentionally thin.  It overrides only the two methods   */
                          ...
                          struct opSet *ops)
{
    ...
    /* DoSuperMethodA invokes drawing.datatype's newDrawing, which   *
     * builds the instance, calls back via DDTM_LOADHEADERS and      *
     * DDTM_LOADCONTENT (both of which arrive at our Dispatch), then *
     * returns the new object pointer.  The callbacks pass the new   *
     * object as the o parameter so we do not need to back-fill any  *
     * localData fields before super runs - INST_DATA returns a      *
     * MEMF_CLEAR slab that already has every flag/stack depth at 0. */
